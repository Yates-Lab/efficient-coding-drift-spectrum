...
\afbcomment{This needs to be written as a SDE.}
\begin{align}
    du(t) = \sqrt{2D} dW(t),\quad u(0) = x_0.
\end{align}
The probability distribution of the SDE solution is $\rho(\xi,\tau)$, where $x_0$ is starting point $\xi$ and $\tau$ is space and time, respectively
\begin{align}
...
\begin{itemize}
    ...
    \item the fall-offs after the ridge for spatial and temporal frequencys are -4 and -2 respectively (if beta=2)
...
\begin{figure}[t]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=14.24cm]{results/optimal_filter.pdf}
    }
