...
Given a Spectrum instance, computes the optimal filter |v*(k, ω)|^2,
...
from src.params import K_MAX, OMEGA_MIN, OMEGA_MAX, hi_res_grid, fast_grid
...
class SolveConfig:
    ...
    P0: float = 50.0
    grid: str = "fast"
    band: tuple = (K_MAX, OMEGA_MIN, OMEGA_MAX)
...
class KernelConfig:
    ...
    @property
    def n_f_fine(self) -> int:
        """Legacy alias for ``n_k_fine``."""
        return self.n_k_fine

    @n_f_fine.setter
    def n_f_fine(self, value: int) -> None:
        self.n_k_fine = int(value)
...
class Result:
    ...
    k_peak: Optional[float] = None  # spatial frequency at peak energy
    ...
    @property
    def f_peak(self) -> Optional[float]:
        """Legacy alias for ``k_peak``."""
        return self.k_peak

    @f_peak.setter
    def f_peak(self, value: Optional[float]) -> None:
        self.k_peak = value
...
def run(
    ...
    grid : "fast" or "hi_res"
        Which (k, ω) grid to use. fast is sufficient for I* sweeps;
        hi_res is required for kernel reconstruction.
    band : tuple
        (k_max, omega_min, omega_max) defining the analysis band.
...
def solve_on_grid(
    ...
    This is the transparent path for interactive scripts: choose ``k`` and
    ``omega`` once, plot spectra on that grid, then solve on the same grid.
    """
    k = np.asarray(k, dtype=float)
    omega = np.asarray(omega, dtype=float)
    k_max, omega_min, omega_max = band
    weights = radial_weights(k, omega)
    mask = band_mask_radial(k, omega, k_max, omega_min, omega_max)
    weights_b = weights * mask

    C = spectrum.C(k, omega)
    ...
    return Result(
        ...
        k=k, omega=omega, C=C, v_sq=v_sq, lam=lam, I=I,
    )
    ...
    for spec in specs:
        ...
        if kernels:
            extract_kernels(
                ...
                n_k_fine=kernel_config.n_k_fine,
...
def extract_temporal_kernel(result: Result,
    ...
    temporal_v, and k_peak on the result."""
    v_sq = result.v_sq
    k = result.k
    omega = result.omega
...
def extract_kernels(result: Result, **kw) -> Result:
    ...
    extract_spatial_kernel(result, **{k: v for k, v in kw.items()
                                      if k in ("k_max", "n_k", "n_k_fine", "n_f_fine")})
...
def spatial_kernel_slice(
    ...
    *,
    ...
    k_fine = np.linspace(0.0, result.k.max() * 1.2, n_k_fine)
    v_interp = np.interp(k_fine, result.k, v_mag_at_w0, left=v_mag_at_w0[0], right=0.0)

    def vmag(k_query):
        return np.interp(k_query, k_fine, v_interp, left=v_interp[0], right=0.0)
...
def temporal_kernel_slice(
    ...
    *,
    ...
    if k0 is None:
        if f0 is None:
            raise TypeError("temporal_kernel_slice requires k0")
        k0 = f0
    i_k0 = int(np.argmin(np.abs(result.k - float(k0))))
    v_mag_at_k0 = np.sqrt(np.maximum(result.v_sq[i_k0, :], 0.0))
    taper = soft_band_taper(result.omega, OMEGA_MIN, OMEGA_MAX, alpha=taper_alpha)
    v_t_smooth = v_mag_at_k0 * taper
    floor = floor_rel * max(v_t_smooth.max(), 1e-30)
    v_t_smooth = np.maximum(v_t_smooth, floor)
