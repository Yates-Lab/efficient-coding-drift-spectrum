...
Our goal is to understand the statistical structure of images formed on moving sensors (such as animal eyes) and understand it's implications on retinal coding. Deriving a complete statistical description of images formed in moving optical systems is difficult. We make the simplifying assumption that the statistics of the movement are stationary in time and independent of the scene being imaged and analyze pair-wise statistics.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=15.41cm]{results/spatial_temporal_spectra_for_different_betas.pdf}
    \includegraphics[width=11.56cm]{results/cross_sections_at_t_and_k.pdf}
    \label{fig:stats}
    \caption{Spatial and temporal spectra for different betas.}
\end{figure}
...
\begin{align}
    u(t) = x_0 + \sqrt{2D}\,W_t.
\end{align}
which has solution $\rho(\xi,\tau)$, where $x_0$ is starting point $x_0$ and $t$ is time
\begin{align}
...
\end{align}
for $D>0$. See appendix~\ref{sec:app_drfit} for derivation details. 
...
Points to make 
\begin{itemize}
    \item Drift $C(k,\omega)$ takes a simple form
    \item the fall-offs for spatial and temporal frequencys are -4 and 0-2 respectively (if beta=2)
    \item The ridge of the spectrum is at $\omega = D k^2$. This is the line at that passes throught kinks in power.
    \item spatial whitening effect of natural images if power spectrum is exactly $1/f^2$. If steeper, then not perfectly whitened. I think people generally find it to be a bit steeper, but marginal. Details aren't that important, just tell people the conditions under which perfect spatial whitening is achieved.   
\end{itemize}
}
...
This finding is quite different from previously reported statistics of natural images (Dong and Atick, Jun et al., ...) who find power law fall off in spatial frequency. Theory sphering/decorrelation in retina predicts circularly symmetric center-surround receptive fields as whitening filters this power law fall-off. 

Function of retina related to the statistics of input. 
Seek to study the data's statistics to gain insight into retinal function. 
...
\end{align}
Here, we assume input and output noise are i.i.d. Gaussian, thus $n_{in}(k,\omega) = \sigma^2_{in}$ and $n_{out}(k,\omega)= \sigma^2_{out}$. Information density per Fourier mode is given by 
\begin{align}
...
\begin{align}
    I(Z;R) = \frac{1}{2} \int_B\frac{dk\,d\omega}{(2\pi)^2}\, \log \left(1+ \frac{v(k,\omega)\, C_z(k,\omega)}{\sigma_{in}^2 v(k,\omega) + \sigma_{out}^2} \right)
\end{align}
...
%     \text{subject to}\quad & \int_B \frac{dk\,d\omega}{(2\pi)^2}\,(1+\eta\omega^2)\,v(k,\omega)\,\big(C_z(k,\omega) + \sigma^2_{in} \big) = 1
% \end{align}
...
\begin{align}
    \int_B \frac{dk\,d\omega}{(2\pi)^2} \left(1+\eta\omega^2\right) C_y(k,\omega)= 1.
\end{align}
Thus the optimization problem is
\begin{align}
    \text{maximize}\quad & I(Z;R) \\
    \text{subject to}\quad & \int_B \frac{dk\,d\omega}{(2\pi)^2} \left(1+\eta\omega^2\right) C_y(k,\omega)= 1 .
\end{align}
...
\begin{figure}[t]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=16cm]{results/model_filter_and_kernels.pdf}
    }
...
\begin{figure}[t]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=14.24cm]{results/model_space_time_noise_both_edit_smaller.pdf}
    }
    \caption{Optimal filter and kernel.}
    \label{fig:scalign}
\end{figure}
...
Points to make 
\begin{itemize}
    \item (figure 2) Optimal filters allocate more power to high spatial frequencies. Still get center-surround receptive fields.
    \item (figure 2) For simple iid noise, drift increases information transfer. The drift reformats spatial information into temporal fluctuations. power constraint and noise makes allocating power to temporal frequencies useful. Result suggests that motion is not a bug, but feature that can serve as a coding strategy for transmitting information about the visual scene. Something thats been said before.
    \afbquestion{Is there empirical evidence that aligns with this prediction -- something to ask Jorge about?}
    \item (figure 2) Retina experiences greater variation in input noise (under vast changes in illumination) than output noise. Model predicts more output noise means less drift. 
...
\subsection{Derivation of Autocorrelation} \label{sec:app_autocorrelation}
...
\subsection{Derivation of ST PSD of Drifting Sensors} \label{sec:app_drfit}
...
\subsection{Derivation of Optimal Linear Filters} \label{sec:app_optimal_filters}
...
% \begin{align}
%     \mathcal{L} = \int_B \frac{dk\,d\omega}{(2\pi)^2} \left[ \frac{1}{2}\log \left(1+ \frac{v(k,\omega)\, C_z(k,\omega)}{\sigma_{in}^2 v(k,\omega) + \sigma_{out}^2} \right) - \lambda\,(1+\eta\omega^2) v(k) \big(C_z(k,\omega) + \sigma_{in}^2\big) \, \right]
% \end{align}
...
% \begin{align}
%     \max_{v\ge 0}\quad 
%     &\frac{1}{2}\int_B\frac{dk\,d\omega}{(2\pi)^2}
%     \log\left(
%     1+
